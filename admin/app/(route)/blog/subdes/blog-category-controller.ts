...
import { BillboardColumn, BlogSubDes, CategoryBlog, CategoryColumn } from "@/types";
...
const get_Blog_Sub_des_Shop = async (): Promise<BlogSubDes[]> => {
  try {
    const response = await fetch(
      linkCustomer.golang_Base +
        linkCustomer.routes_blog_sub_des.getAll,
      {
        method: "GET",
        cache: "no-store"
      }
