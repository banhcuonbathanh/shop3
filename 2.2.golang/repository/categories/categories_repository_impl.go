...
func (c *CategoryRepositoryImpl) FindAll() []model.Category {
    fmt.Printf("from the categories repository:asdfhksajdhfk")
//         err := c.Db.Exec(`
//     ALTER TABLE categories ADD CONSTRAINT fk_categories_product_id
...
// }
	...
	results := c.Db.Preload("Product").Find(&categories)
	helper.ErrorPanic(results.Error)
    fmt.Printf("from the categories repository: %+v\n", categories)
	return categories
}
